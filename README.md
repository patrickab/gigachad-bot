...
Web Search mode is backed by **Vane** (a Perplexica fork) running in Docker. It
**auto-starts on your first web-search request** — no manual step for Ollama or
Gemini models. To start it by hand instead:

```bash
docker compose -f docker-compose.vane.yml up -d --wait
```
