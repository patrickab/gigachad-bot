...
the request's user. Only PDF library assets are mirrored into Nextcloud so
MinerU can parse a real file. All other application state remains
database-only.
"""
...
def _mirror(assets: AssetStore, asset: Asset) -> Path | None:
    """Mirror an allowed PDF or MinerU asset into Nextcloud.
...
def _library_asset(assets: AssetStore, name: str, content: bytes) -> Asset:
    """Store an eligible PDF in the library and prepare it for MinerU."""
    if Path(name).suffix.lower() != ".pdf":
        raise HTTPException(status_code=400, detail="Only PDF documents can enter the PDF library")
    return assets.write("pdf", f"{KEY_PDFS}/{name}", content, mime="application/pdf")
